...
void VirtualWP::generate(const AP_Mission::Mission_Command& cmd)
{
	...
	loc_vwp1.alt = land_wp.alt; // + 3*step; // same altitude as landing waypoint
	...
	// http://ardupilot.org/plane/docs/common-mavlink-mission-command-messages-mav_cmd.html#mav-cmd-nav-loiter-to-alt
	...
	// I loiter right after reaching vwp 2.
	lta.content.location.lat = loc_vwp2.lat;
	lta.content.location.lng = loc_vwp2.lng;
	lta.content.location.alt = land_wp.alt;
	// Heading aligned with the direction of the next virtual waypoint
	// Radius of 40 meters. This should be set as additional parameter
	uint8_t radius_m = 40;
